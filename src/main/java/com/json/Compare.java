						...
						if (jObj.length() == jObj2.length()) {
							...
							while (iterator.hasNext()) {
								...
								if ("OpsStart".equalsIgnoreCase(key) || "OpsEnd".equalsIgnoreCase(key)
									...
									if (!jObj.getString(key).substring(0, (jObj.getString(key)).indexOf("T"))
											.equalsIgnoreCase(jObj2.getString(key).substring(0,
													(jObj2.getString(key)).indexOf("T")))) {
										compareResult.fail("Data mismatching at index \"" + itr
												+ "\" with the key \"" + key + "\".",
												jObj.getString(key).substring(0, (jObj.getString(key)).indexOf("T")),
												jObj2.getString(key).substring(0, (jObj2.getString(key)).indexOf("T")));
									}
								} else if (!(jObj.getString(key)).equalsIgnoreCase(jObj2.getString(key))) {
									compareResult.fail("Data mismatching at index \"" + itr
											+ "\" with the key \"" + key + "\".", jObj.getString(key),
											jObj2.getString(key));
								} else {
